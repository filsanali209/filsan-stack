...
export class BakehouseStack extends Stack {
 ...
 // ----------------------------------
    ...
    //
    // PINCH POINT — provider already exists:
    //   Each AWS account can only have ONE OIDC provider per URL. If you
    //   previously created this manually in the Console or via a previous
    //   stack, `cdk deploy` will fail with:
    //   "An OpenID Connect provider with this URL already exists"
    //
    //   Fix — delete the old one first:
    //     Go to IAM → Identity providers → token.actions.githubusercontent.com
    //     → Delete. Then re-run cdk deploy.
    //
    //   Or import it instead of creating it (see README troubleshooting).
    // ─────────────────────────────────────────────────────────────────────────
  ...
  `arn:aws:iam::${this.account}:oidc-provider/token.actions.githubusercontent.com`
    ...
    //       },
    //       StringLike: {
    //         // Matches any branch or environment in your repo.
    //         // Tighten this to e.g. "ref:refs/heads/GH-Actions" for production.
  ...
  const githubOrg = props.githubOrg || 'filsanali209'
  const githubRepo = props.githubRepo || 'filsan-stack'
  const githubRefFilter = props.githubRefFilter || 'ref:refs/heads/main'

  const role = new iam.Role(this, "GitHubActionsRole", {
  roleName: `github-actions-filsan-filsan-stack`, // 👈 yourName added here
  assumedBy: new iam.WebIdentityPrincipal(
    provider.openIdConnectProviderArn,
    {
      StringEquals: {
