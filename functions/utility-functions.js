...
export const postProductHandler = async (event) => {
  ...
  try {
    ...
    // We store the PDF *key* only, not the full S3 URL
    // RETURNING lets us immediately get the inserted row back
    const insertSql = `
    ...
    // Run the SQL query and pass in the values safely as parameters
    // This avoids SQL injection and keeps the query clean
    const insertResult = await runQuery(insertSql, {
    name,                 // product name
    description,          // product description
    price_pence,          // price stored in pence to avoid float issues
    pdf_url: pdfKey,      // store only the S3 key, not a full URL
    })

    // Different database drivers return results in different shapes
    // This safely grabs the first returned row no matter the format
    const row =
    insertResult?.records?.[0] ||
    insertResult?.rows?.[0] ||
    insertResult?.[0]

    // Build a clean product object to return or send back to the client
    // This controls exactly what data leaves this function
    const product = {
    id: row.id,                 // database generated id
    name: row.name,
    description: row.description,
    price_pence: row.price_pence,
    category,                   // comes from earlier logic
    pdf_url: pdfKey,            // keep the key here, not the full URL
    }
...
export const getCustomersHandler = async (event, context) => {
  ...
  try {
    const result = await runQuery(
      //TODO - Replace this comment with an SQL statement that will get all customers
      `SELECT c.id, c.name, c.email, o.id AS order.id
      FROM customers c 
      LEFT JOIN orders o 
...
export const postCustomersHandler = async event => {
  ...
  try {
    ...
    // Run the INSERT against the database
    const result = await runQuery(
      // Add POST SQL Here
    );

    // If a row was created, it will be in result.records[0]
    const created =
      (result.records && result.records[0]) ||
      { name, email, note: "already existed" };

    // Return a 201 so the client knows it succeeded
